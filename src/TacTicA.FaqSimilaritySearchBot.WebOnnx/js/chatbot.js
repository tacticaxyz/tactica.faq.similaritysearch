// TacTicA.FaqSimilaritySearchBot JavaScript Client (ONNX-enabled)
// Changes:
// - Single async user embedding via getUserEmbedding used for FAQ + RAG
// - findBestFAQMatch / findRelevantDocuments accept precomputed embedding
// - Dimension mismatch safeguards & graceful fallback
// - Config-derived embedding dimension / max sequence length
// - Added minor diagnostics
...
class TacTicAFaqSimilaritySearchBot {
    constructor() {
        ...
        this.loadData(); // async but not awaited here
    ...
    async initializeOnnx() {
        if (this.embeddingProvider !== 'onnx') return;
        if (typeof ort === 'undefined') {
            console.warn('ONNX runtime not loaded, falling back to simple embeddings.');
            this.embeddingProvider = 'simple';
            return;
        }
        try {
            this.showSystemMessage('Loading ONNX model...', 'info');
            this.onnx.session = await ort.InferenceSession.create('models/model.onnx', {
                executionProviders: ['wasm'],
                graphOptimizationLevel: 'all'
            });
            this.onnx.vocab = await this.loadVocab();
            this.showSystemMessage('✅ ONNX model ready.', 'success');
            console.log('ONNX vocab size:', Object.keys(this.onnx.vocab).length);
        } catch (err) {
            console.error('Failed to load ONNX model, fallback to simple embeddings:', err);
            this.embeddingProvider = 'simple';
            this.onnx.loadingFailed = true;
            this.showSystemMessage('ONNX model load failed. Using simple embeddings.', 'error');
        }
    }

    async loadVocab() {
        try {
            const resp = await fetch('models/model_vocab.txt');
            if (resp.ok) {
                const text = await resp.text();
                const lines = text.split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 0);
                const vocab = {};
                lines.forEach((tok, idx) => vocab[tok] = idx);
                if (vocab['[PAD]'] !== undefined) this.onnx.padId = vocab['[PAD]'];
                if (vocab['[UNK]'] !== undefined) this.onnx.unkId = vocab['[UNK]'];
                if (vocab['[CLS]'] !== undefined) this.onnx.clsId = vocab['[CLS]'];
                if (vocab['[SEP]'] !== undefined) this.onnx.sepId = vocab['[SEP]'];
                return vocab;
            }
        } catch {}
        try {
            const resp = await fetch('models/model_tokenizer.json');
            if (resp.ok) {
                const json = await resp.json();
                const vocabObj = json?.model?.vocab || {};
                return vocabObj;
            }
        } catch {}
        console.warn('No vocab file found, using minimal fallback vocab');
        return {
    ...
    async getUserEmbedding(text) {
        if (this.embeddingProvider === 'onnx' && this.onnx.session && this.onnx.vocab) {
            try {
                return await this.generateOnnxEmbedding(text);
            } catch (e) {
                console.warn('ONNX embedding failed, fallback to simple', e);
    ...
    tokenizeForOnnx(text) {
        const norm = text.toLowerCase().trim().replace(/[^\w\s]/g, ' ');
        return norm.split(/\s+/).filter(t => t.length > 0);
    }

    tokensToIds(tokens) {
        const vocab = this.onnx.vocab;
        const ids = [this.onnx.clsId];
        for (const tok of tokens) {
            ids.push(vocab[tok] !== undefined ? vocab[tok] : this.onnx.unkId);
        }
        ids.push(this.onnx.sepId);
        return ids;
    }

    async generateOnnxEmbedding(text) {
        const tokens = this.tokenizeForOnnx(text);
        let ids = this.tokensToIds(tokens);
        const maxLen = this.onnx.maxLength;
        if (ids.length > maxLen) {
            ids = ids.slice(0, maxLen);
            ids[ids.length - 1] = this.onnx.sepId;
        }
        while (ids.length < maxLen) ids.push(this.onnx.padId);

        const attentionMask = ids.map(id => id === this.onnx.padId ? 0 : 1);
        const tokenTypeIds = new Array(maxLen).fill(0);

        const shape = [1, maxLen];
        // Prefer int32 if model inputs are int32; fallback to int64
        const useInt32 = this.onnx.session.inputNames.some(n => {
            const meta = this.onnx.session.inputMetadata?.[n];
            return meta && meta.type === 'int32';
        });

        const buildTensor = (arr) => {
            if (useInt32) return new ort.Tensor('int32', Int32Array.from(arr), shape);
            return new ort.Tensor('int64', BigInt64Array.from(arr.map(i => BigInt(i))), shape);
        };

        const feeds = {
            input_ids: buildTensor(ids),
            attention_mask: buildTensor(attentionMask)
        };
        if (this.onnx.session.inputNames.includes('token_type_ids')) {
            feeds.token_type_ids = buildTensor(tokenTypeIds);
        }

        const results = await this.onnx.session.run(feeds);
        const firstKey = this.onnx.session.outputNames[0];
        const output = results[firstKey].data;
        const hiddenSize = output.length / this.onnx.maxLength;
        const pooled = new Float32Array(hiddenSize);
        let validCount = 0;
        for (let i = 0; i < this.onnx.maxLength; i++) {
            if (attentionMask[i] === 1) {
                for (let h = 0; h < hiddenSize; h++) {
                    pooled[h] += output[i * hiddenSize + h];
                }
                validCount++;
            }
        }
        if (validCount > 0) {
            for (let h = 0; h < hiddenSize; h++) pooled[h] /= validCount;
        }
        let mag = 0;
        for (let h = 0; h < hiddenSize; h++) mag += pooled[h] * pooled[h];
        mag = Math.sqrt(mag) || 1;
        for (let h = 0; h < hiddenSize; h++) pooled[h] /= mag;
        ...
        try {
            ...
            try {
                ...
                if (qaResponse.ok && chunksResponse.ok && configResponse.ok) {
                    ...
                    // Derive embedding dimension and max length if available
                    if (this.config?.embeddingDimension) {
                        this.onnx.embeddingDimension = this.config.embeddingDimension;
                    } else if (this.qaData?.length && this.qaData[0]?.embedding) {
                        this.onnx.embeddingDimension = this.qaData[0].embedding.length;
                    }
                    if (this.config?.maxSequenceLength) {
                        this.onnx.maxLength = this.config.maxSequenceLength;
                    }

                    console.log('Data loaded successfully via fetch:', {
                        qaCount: this.qaData?.length || 0,
                        chunkCount: this.documentChunks?.length || 0,
                        embeddingDimension: this.onnx.embeddingDimension,
                        maxSeq: this.onnx.maxLength
                    });

                    this.showSystemMessage(`✅ Chatbot ready (embeddings: ${this.onnx.embeddingDimension}d, provider: ${this.embeddingProvider}).`, 'success');
                    return;
                }
            } catch (fetchError) {
                console.log('Fetch failed, fallback to embedded data:', fetchError.message);
            }
    ...
    async handleSendMessage() {
        ...
        // Show typing indicator and status indicator
        console.log('Starting message processing...');
        this.setLoading(true);
        this.addTypingIndicator();

        // CRITICAL: Force UI update and yield control to browser before heavy computation
        await new Promise(resolve => setTimeout(resolve, 10));

        try {
            console.log('Calling processMessage...');
            const response = await this.processMessage(message);
            console.log('processMessage completed, hiding indicators...');
            
            // Hide indicators BEFORE showing response
    ...
    async processMessage(userQuestion) {
        ...
        // Compute embedding once (async if ONNX)
        let userEmbedding = null;
        try {
            userEmbedding = await this.getUserEmbedding(userQuestion);
        } catch (e) {
            console.warn('Embedding generation failed, fallback simple.', e);
            userEmbedding = this.generateSimpleEmbedding(userQuestion);
        }
        // Step 1: Try FAQ matching
        const faqResult = this.findBestFAQMatch(userEmbedding);
        const threshold = this.config?.defaultSimilarityThreshold || 0.85; // Lowered from 0.90

        if (faqResult.score >= threshold && faqResult.qa) {
            // FAQ hit - get documents from the FAQ answer
            console.log('FAQ match found:', faqResult);
            const relevantChunks = this.findRelevantDocuments(userEmbedding, userQuestion, faqResult.qa.wikiLinks);
            return {
                isValid: true,
                isRag: false,
            ...
            const relevantChunks = this.findRelevantDocuments(userEmbedding, userQuestion);
            return {
                isValid: true,
                isRag: true,
    ...
    findBestFAQMatch(userEmbedding) {
        if (!userEmbedding) return { qa: null, score: -1 };
        let bestMatch = null;
        let bestScore = -1;

        for (const qa of this.qaData) {
            if (!qa.embedding || qa.embedding.length === 0) continue;
            if (qa.embedding.length !== userEmbedding.length) {
                // Dimension mismatch (likely different embedding backend) - skip
                continue;
            }
    ...
    findRelevantDocuments(userEmbedding, userQuestion, specificUrls = null, topK = 5) {
        ...
        const scored = candidates
            .filter(chunk => chunk.embedding && chunk.embedding.length === userEmbedding.length)
            .map(chunk => {
        ...
        console.log(`Relevant docs: ${scored.length}/${candidates.length}`, 
          scored.slice(0, 3).map(s => ({
            title: s.title,
            score: s.score.toFixed(3),
            vs: s.vectorScore.toFixed(3),
            ts: s.textScore.toFixed(3)
        })));
        return scored;
    }
...
Is there a different way I can help you with your [Chromium codebase] question?`];
    ...
    generateSimpleEmbedding(text) {
        const dimension = this.onnx.embeddingDimension || 384; // Match the training service
        ...
        let mag = 0;
        for (let v of embedding) mag += v * v;
        mag = Math.sqrt(mag) || 1;
        for (let i = 0; i < embedding.length; i++) embedding[i] /= mag;
        return embedding;
    }
    ...
    setLoading(loading) {
        console.log('setLoading called with:', loading, 'at', new Date().toISOString());
        this.isLoading = loading;
        this.sendButton.disabled = loading;
        this.userInput.disabled = loading;
        
        if (loading) {
            console.log('Showing status indicator');
            this.statusIndicator.classList.remove('hidden');
            this.sendButton.classList.add('loading');
            
            // Force a reflow to ensure the change takes effect
            this.statusIndicator.offsetHeight;
            
            // Log the element state
            setTimeout(() => {
                console.log('Status indicator after showing:', {
                    hidden: this.statusIndicator.classList.contains('hidden'),
                    visible: !this.statusIndicator.classList.contains('hidden'),
                    rect: this.statusIndicator.getBoundingClientRect()
                });
            }, 10);
        } else {
